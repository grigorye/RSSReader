...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="FPSCounter",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
/// <code>FPSCounter</code> uses <code>CADisplayLink</code> updates to count the frames per second drawn.
...
/// <code>FPSCounter.showInStatusBar(_:)</code> convenience method.
SWIFT_CLASS("_TtC10FPSCounter10FPSCounter")
@interface FPSCounter : NSObject
/// Create a new FPSCounter.
/// To start receiving FPS updates you need to start tracking with the
/// <code>startTracking(inRunLoop:mode:)</code> method.
...
/// (default), or the tracking mode (<code>UITrackingRunLoopMode</code>).
/// When the counter is already tracking, it’s stopped first.
/// \param runloop The runloop to start tracking in
///
/// \param mode The mode(s) to track in the runloop
///
...
@interface FPSCounter (SWIFT_EXTENSION(FPSCounter))
...
/// \param application The <code>UIApplication</code> to show the FPS for
///
/// \param runloop The <code>NSRunLoop</code> to use when tracking FPS or <code>nil</code> (then it uses the main run loop)
///
/// \param mode The run loop mode to use when tracking. If <code>nil</code> it uses <code>NSRunLoopCommonModes</code>
///
+ (void)showInStatusBar:(UIApplication * _Nonnull)application runloop:(NSRunLoop * _Nullable)runloop mode:(NSRunLoopMode _Nullable)mode;
