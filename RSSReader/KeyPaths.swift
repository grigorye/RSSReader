...
private func recordKeyPath<T: NSObject, V>(valueClass: AnyClass!, x: T, @noescape recorder: (T) -> V) -> String {
	...
	let objectClass: AnyClass = object_setClassAndRetain(x, KeyPathRecordingProxy.self)
	recorder(x)
	object_setClassAndRetain(x, objectClass)
	objc_setAssociatedObject(x, keyPathRecorderProxyAssociation, nil, UInt(OBJC_ASSOCIATION_ASSIGN))
